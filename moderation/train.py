"""
Phase 1 - Classifier & Data Preparation.
...
Label convention (matches the protocol's C_theta(f) = 1 == "passes filter"):
    label 1 = ham   (benign / allowed to send)
    label 0 = spam  (blocked)

The circuit will later prove:  theta_q . phi(m) + bias_q  >=  tau_q   (== ham).

Outputs
-------
    moderation/models/model_d{64,256,1024}.json   quantised model + metrics
...
def evaluate(y_true, y_pred):
    # spam is the moderation-relevant positive class -> pos_label = 0 (spam)
    return {
...
def main():
    ...
    combined = {"scale": SCALE, "seed": SEED, "dims": {}}
    per_dim_models = {}
    ...
    for d in DIMS:
        ...
        tau_q = 0   # threshold folded into the bias; kept explicit for the circuit
    ...
    print("\nWrote models + metrics + test_vectors to", OUT)
